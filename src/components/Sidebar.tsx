...
export function Sidebar({
    ...
    const dbPlaces = React.useMemo(() => {
        const merged = uniqById([...(approvedLocations ?? []), ...(userLocations ?? [])]);
    ...
    const allPlaces: SuggestedPlace[] = React.useMemo(() => {
        return uniqById([...(dbPlaces ?? []), ...((suggested ?? []) as SuggestedPlace[])]);
    }, [dbPlaces, suggested]);
    ...
    const fromMatches = React.useMemo(() => {
        ...
        return allPlaces.filter((p) => p.name.toLowerCase().includes(q)).slice(0, 60);
    }, [fromQuery, allPlaces]);
            ...
            >
                        ...
                        <div className="flex items-center gap-3">
                            <div className="flex items-center gap-2">
                                ...
                                <div className="leading-tight">
                                    ...
                                    <div className="text-white/55 text-xs -mt-0.5">Suggested Places</div>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                >
                                    <div className="flex gap-3">
                                        <div className="min-w-0 flex-1">
                                            ...
                                            {p.category && (
                                                <div className="text-white/55 text-xs truncate mt-0.5">{p.category}</div>
                                            )}
                                            ...
                                            <div className="flex items-center gap-2 mt-1">
                                                ...
                                                {typeof p.distanceKm === "number" && (
                                                    <div className="text-xs text-white/55">{p.distanceKm.toFixed(1)} km</div>
                                                )}
                                            </div>
                ...
                <GlassCard className="flex flex-col min-h-0">
                    <div className="px-4 pt-4">
                        <div className="flex items-center gap-2">
                            <ModeBtn active={mode === "drive"} onClick={() => setMode("drive")}>
                                <Car className="h-5 w-5" />
                            </ModeBtn>
                            <ModeBtn active={mode === "walk"} onClick={() => setMode("walk")}>
                                <PersonStanding className="h-5 w-5" />
                            </ModeBtn>
                            <ModeBtn active={mode === "transit"} onClick={() => setMode("transit")}>
                                <Bus className="h-5 w-5" /> {/* 👈 CHANGE BUS ICON SIZE HERE (e.g. h-6 w-6) */}
                            </ModeBtn>
                        </div>
                    </div>
                        ...
                        >
                            {!fromPlace || !toPlace ? (
                                <div className="text-xs text-white/55">
                                    ...
                                    <span className="text-white">To</span> location to see travel time.
                                </div>
                            ) : loadingRoutes ? (
                                ...
                                <>
                                    <div className="text-[11px] text-white/55 mb-2">Fastest Route</div>
                                    {fastest ? <RouteCard item={fastest} accent={ACCENT} /> : null}

                                    <div className="text-[11px] text-white/55 mt-4 mb-2">Alternative Routes</div>
                                    <div className="space-y-2">
                                        {alternatives.map((r) => (
                                            <RouteCard key={r.id} item={r} accent={ACCENT} />
                                        ))}
                                        {alternatives.length === 0 && (
                                            <div className="text-xs text-white/55">No alternatives found.</div>
                                        )}
                                    </div>

                                    {mode === "transit" && (
                                        <div className="text-[11px] text-white/45 mt-3">
                                            Bus time is an estimate (no timetables). Real transit needs GTFS/OTP/Google Routes.
                                        </div>
                                    )}
                                </>
                            )}
                    ...
                    <div className="max-h-[420px] overflow-y-auto">
                        {options.map((p) => (
                            ...
                            >
                                ...
                                {p.category && (
                                    <div className="text-xs text-white/55 truncate">{p.category}</div>
                                )}
                            </button>
                        ))}
                    </div>
