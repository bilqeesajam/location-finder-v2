...
export function MapView({
  ...
  const [trainRouteVisible, setTrainRouteVisible] = useState(false);
  ...
  useEffect(() => {
    const handler = (ev: Event) => {
      const e = ev as CustomEvent<{ mode: string; trainRouteVisible?: boolean }>;
      if (!e.detail) return;
      setCurrentMode(e.detail.mode as any);
      if (e.detail.trainRouteVisible !== undefined) {
        setTrainRouteVisible(e.detail.trainRouteVisible);
      }
    };
...
if (!showTrainRoute) {
  ...
  }, [isLoaded, currentMode, trainRouteVisible]);
  ...
  useEffect(() => {
    ...
    const showTrain = currentMode === "train" && trainRouteVisible;
